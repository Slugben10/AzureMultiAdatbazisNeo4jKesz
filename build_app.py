...
def create_windows_neo4j_conf():
    """Create a Windows-compatible Neo4j configuration file template."""
    print("Creating Windows-compatible Neo4j configuration template...")
    conf_template = """# Neo4j configuration template for Windows
...
"""
    ...
    # Write the template to a file in the hooks directory
    with open(os.path.join("hooks", "neo4j.conf.template"), "w") as f:
        f.write(conf_template)
    print("Neo4j configuration template created")
...
# Create Windows-specific Neo4j configuration template
if sys.platform == 'win32':
    create_windows_neo4j_conf()
...
if sys.platform == 'win32' and os.path.exists(os.path.join("hooks", "neo4j.conf.template")):
    data_files.append((os.path.join("hooks", "neo4j.conf.template"), "Neo4jDB/conf/"))
...
try:
    ...
    if sys.platform == 'win32':
        print("Performing Windows-specific post-build steps...")
        
        # Create Neo4j server directories in the output folder
        dist_dir = os.path.join('dist', APP_NAME)
        neo4j_dirs = [
            os.path.join(dist_dir, 'Neo4jDB', 'data'),
            os.path.join(dist_dir, 'Neo4jDB', 'logs'),
            os.path.join(dist_dir, 'Neo4jDB', 'conf'),
            os.path.join(dist_dir, 'Neo4jDB', 'neo4j-server', 'conf'),
            os.path.join(dist_dir, 'Neo4jDB', 'plugins'),
            os.path.join(dist_dir, 'Neo4jDB', 'lib'),
            os.path.join(dist_dir, 'Neo4jDB', 'run'),
            os.path.join(dist_dir, 'Neo4jDB', 'metrics'),
            os.path.join(dist_dir, 'Neo4jDB', 'data', 'transactions'),
        ]
        
        for directory in neo4j_dirs:
            os.makedirs(directory, exist_ok=True)
            print(f"Created Neo4j directory: {directory}")
        
        # Copy Neo4j configuration template
        neo4j_conf_template = os.path.join('hooks', 'neo4j.conf.template')
        if os.path.exists(neo4j_conf_template):
            target_conf_dir = os.path.join(dist_dir, 'Neo4jDB', 'neo4j-server', 'conf')
            shutil.copy(neo4j_conf_template, os.path.join(target_conf_dir, 'neo4j.conf'))
            print(f"Copied Neo4j configuration template to {target_conf_dir}")
        
        # Create a marker file to indicate this is a bundled Neo4j
        marker_path = os.path.join(dist_dir, 'Neo4jDB', '.bundled')
        with open(marker_path, 'w') as f:
            f.write('This directory contains a Neo4j database bundled with the application.')
        
        # Create a marker file to preserve data between runs
        preserve_path = os.path.join(dist_dir, 'Neo4jDB', '.preserve')
        with open(preserve_path, 'w') as f:
            f.write('This file indicates that Neo4j data should be preserved between application runs.')
        
        print("Windows-specific post-build steps completed successfully")
...
""")
    ...
    if sys.platform == 'darwin':
        ...
        print(f"1. For best results, use the '{APP_NAME}_launcher.bat' file to start the application")
        print(f"2. This launcher ensures proper environment setup before starting the app")
        print(f"3. If you encounter path-related issues, check that file paths don't contain special characters")
    print(f"4. Make sure Neo4j and Java are properly bundled or will be downloaded on first run")
